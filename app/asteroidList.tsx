...
const AsteroidItem = ({
  ...
  <div>
    ...
    <div className={`asteroid-details ${isExpanded ? 'open' : ''}`}>
      {isExpanded && <AsteroidDetails asteroid={asteroid} />}
    </div>
  </div>
);

const AsteroidDetails = ({ asteroid }: { asteroid: Asteroid }) => (
  <div className="mt-2 rounded-lg bg-gray-800 p-4 text-white">
    ...
    <ul className="ml-4 list-disc">
      ...
      <li>
        Meters: {asteroid.estimated_diameter.meters.estimated_diameter_min} -{' '}
        {asteroid.estimated_diameter.meters.estimated_diameter_max}
      </li>
      <li>
        Miles: {asteroid.estimated_diameter.miles.estimated_diameter_min} -{' '}
        {asteroid.estimated_diameter.miles.estimated_diameter_max}
      </li>
      <li>
        Feet: {asteroid.estimated_diameter.feet.estimated_diameter_min} -{' '}
    ...
    <p>
      <strong>Sentry Object:</strong> {asteroid.is_sentry_object ? 'Yes' : 'No'}
    </p>
  </div>
);
      ...
      {asteroids.length === 0 ? (
        ...
        asteroids.map((asteroid) => (
          <AsteroidItem
            key={asteroid.id}
            asteroid={asteroid}
            isExpanded={expandedAsteroids.has(asteroid.id)}
